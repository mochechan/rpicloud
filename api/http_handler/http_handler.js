...
var log; // for code self-contained
...
"/upload_form": function (request, response, _rc) {
	log = _rc.log;
...
"/upload_file": function (request, response, _rc) {
	log = _rc.log;
	var that = this;
  log("/upload_file was called.");
  var form = new formidable.IncomingForm();
	form.encoding = 'utf-8'; //Sets encoding for incoming form fields.
	form.uploadDir = os.tmpDir(); //Sets the directory for placing file uploads in. You can move them later on using fs.rename(). The default is os.tmpDir().
		...
		if (Array.isArray(files.upload)) {
			if (fields.preserveFileName === 'true' ) {
				for (var i in files.upload) {
					...
					fs.rename(files.upload[i].path, path.resolve(form.uploadDir, files.upload[i].name), function (err) {
						if (err) {
			...
			if (fields.preserveFileName === 'true' ) {
	  	  fs.rename(files.upload.path, path.resolve(form.uploadDir, files.upload.name), function(err) {
  	  	  if (err) {
						log("ERROR: file upload rename fails");
	  	    }
		    });
...
"/upload_finish": function (request, response, _rc) {
	log = _rc.log;
...
"/user_login": function (request, response, _rc) {
	log = _rc.log;
	log("http_handler.js is calling /user_login");
